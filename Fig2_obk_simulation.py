...
def obk_simu(iniEPI_file,tmpKt,Rt_srs,mean_GT, sd_GT,simuK):
    ...
    July_idx = 62;
    Aug_idx = 92;
    simu_result = np.zeros((len(Rt_srs)+1,7));
    ...
    for rt_id in range(len(Rt_srs)):
        ...
        simu_result[rt_id,4:] = np.percentile(Epi_size_Aug,[50, 25, 75])
    simu_result[-1,:] = np.array([21,62,62,62,92,92,92]);

    acc_case = np.cumsum(initialsrs);
    rpt_date = np.arange(0,len(initialsrs));

    late_case = np.array([45+81,45+81+54])
    late_date = np.array([62,92])
...
for k_id in range(len(k_srs)):
    ...
    tmpK = k_srs[k_id];
    acc_case,rpt_date,simu_result = obk_simu(iniEPI_file,tmpK,Rt_srs,mean_GT, sd_GT,simuK);    
    tmpAx.plot(rpt_date,acc_case,c='k',linewidth = 1.5*lw);
    for rt_id in range(len(Rt_srs)):
        tmpAx.plot(simu_result[-1,[0,1,4]],simu_result[rt_id,[0,1,4]],linewidth = lw);
    tmpAx.set_xticks([0,21,62,92])
    tmpAx.set_yticks([50,150,250])
    if k_id%2 == 1:
